"""Add WebRTC meeting tables (custom_meetings, participants, transcripts)
...
def upgrade():
    conn = op.get_bind()
    if not _table_exists(conn, "custom_meetings"):
        op.create_table(
            "custom_meetings",
        ...
        op.create_index(
            op.f("ix_custom_meetings_id"), "custom_meetings", ["id"], unique=False
        )
        op.create_index(
            op.f("ix_custom_meetings_meeting_id"),
            "custom_meetings",
            ["meeting_id"],
            unique=True,
        )
    ...
    if not _table_exists(conn, "meeting_participants"):
        ...
        op.create_index(
            op.f("ix_meeting_participants_id"),
            "meeting_participants",
            ["id"],
            unique=False,
        )

    if not _table_exists(conn, "custom_meeting_transcripts"):
        op.create_table(
        ...
        op.create_index(
            op.f("ix_custom_meeting_transcripts_id"),
            "custom_meeting_transcripts",
            ["id"],
            unique=False,
        )
...
def downgrade():
    op.drop_index(
        op.f("ix_custom_meeting_transcripts_id"),
        table_name="custom_meeting_transcripts",
    )
