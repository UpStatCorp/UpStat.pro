...
def _index_exists(conn, name: str) -> bool:
    try:
        for tbl in sa_inspect(conn).get_table_names():
            for idx in sa_inspect(conn).get_indexes(tbl):
...
def upgrade():
    ...
    if not _table_exists(conn, 'crm_deals'):
        op.create_table(
            ...
            sa.Column('closed', sa.Boolean(), server_default=sa.false(), nullable=False),
    ...
    if not _table_exists(conn, 'crm_leads'):
        op.create_table(
            ...
            sa.Column('assigned_by_id', sa.Integer(), nullable=True),
            sa.Column('assigned_by_name', sa.String(255), nullable=True),
            sa.Column('converted', sa.Boolean(), server_default=sa.false(), nullable=False),
    ...
    if not _index_exists(conn, 'ix_crm_recordings_deal_id'):
        op.create_index('ix_crm_recordings_deal_id', 'crm_recordings', ['deal_id'])
    if not _index_exists(conn, 'ix_crm_recordings_lead_id'):
        op.create_index('ix_crm_recordings_lead_id', 'crm_recordings', ['lead_id'])
    if not _index_exists(conn, 'ix_crm_recordings_contact_crm_id'):
