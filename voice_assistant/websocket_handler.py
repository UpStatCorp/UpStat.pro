...
# Ведение тренировки. Без этого блока переход между этапами превращался
# в четыре пустые реплики подряд: «Мы переходим к следующему этапу… Готов?» →
# «Отлично, сейчас расскажу, что будет дальше» → «Я предложу тебе продукт…
# Готов попробовать?» — пользователь трижды отвечал «готов», а тренировка
# так и не начиналась. Модель по своей природе анонсирует действие вместо
# того, чтобы его выполнить, и переспрашивает разрешение на каждом шаге.
_STAGE_FLOW_RULE = (
    "\n===== ВЕДЕНИЕ ТРЕНИРОВКИ =====\n"
    "Не анонсируй то, что собираешься сделать — сразу делай.\n"
    "ЗАПРЕЩЕНО:\n"
    "- Спрашивать готовность или разрешение: «Готов?», «Готов попробовать?», "
    "«Начнём?», «Можем начинать?», «Давай начнём?».\n"
    "- Реплики-пустышки, в которых нет ни фразы в роли, ни конкретного вопроса "
    "по тренировке: «Сейчас расскажу, что будет дальше», «Давай начнём наш новый сценарий».\n"
    "- Повторять анонс, который уже прозвучал.\n"
    "- Объявлять смену этапа отдельной репликой.\n"
    "В КАЖДОЙ твоей реплике должно быть само тренировочное действие: либо фраза "
    "в роли (менеджера или клиента), либо конкретный вопрос пользователю по тренировке.\n"
    "Объяснение и действие — в ОДНОЙ реплике: максимум 1-2 фразы объяснения, "
    "дальше сразу первая фраза сценария.\n"
    "Если пользователь сказал «да», «готов», «давай» — не благодари и не анонсируй, "
    "а сразу продолжай сценарий следующей фразой в роли.\n"
    "Пользователь должен всегда понимать, что делать дальше: заканчивай реплику "
    "либо своей фразой в роли, либо прямым указанием, что ему сказать или сделать.\n"
)

# Правила, действующие на КАЖДУЮ реплику. Собраны в одну константу, потому что
# response.instructions замещает инструкции сессии целиком: где бы мы ни
# формировали инструкции — старт, переход, триггер реплики — правила должны
# уехать вместе с промптом, иначе для этой реплики их просто нет.
_DIALOG_RULES = _RU_OUTPUT_RULE + _STAGE_FLOW_RULE
...
async def handle_websocket_connection(
    ...
    try:
        ...
        session_instructions = session_instructions + _DIALOG_RULES
...
async def _handle_stage_action(
    ...
    try:
        ...
        if action == "next_stage":
            ...
            user_session.session_instructions = stage_prompt + _DIALOG_RULES
            ...
            try:
                # Служебность подчёркнута намеренно: без этого модель
                # пересказывала напоминание вслух отдельной репликой
                # («Мы переходим к следующему этапу, теперь я менеджер…»),
                # и этап начинался только через два-три обмена.
                system_reminder = (
                    f"⚠️ СЛУЖЕБНОЕ СООБЩЕНИЕ. СМЕНА КОНТЕКСТА ТРЕНИРОВКИ — "
                    f"ЭТАП {next_stage.number} ИЗ {len(user_session.stages)}.\n\n"
                    f"{role_reminder}\n\n"
                    ...
                    f"Это сообщение НЕ нужно проговаривать вслух и пересказывать. "
                    f"Не объявляй смену этапа и не описывай новые роли отдельной репликой — "
                    f"переход должен уместиться в одну короткую связку внутри первой "
                    f"фразы этапа, после которой сразу идёт действие."
                )
            ...
            try:
                ...
                if next_stage.first_line_template:
                    ...
                    stage_task = (
                        ...
                        "НЕ спрашивай готовность и НЕ анонсируй, что будет дальше — "
                        "реплика обязана заканчиваться фразой в роли или прямым указанием, "
                ...
                # _DIALOG_RULES обязательны и здесь: response.instructions
                # замещает инструкции сессии, поэтому без них ПЕРВАЯ реплика
                # каждого этапа генерировалась без правил языка и тона — отсюда
                # запрещённые «Отлично, начнём» ровно на переходах.
